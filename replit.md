...
- **Document Translation:** Users can translate processed documents into 30+ supported languages (via DeepL). The "Translate Document" panel appears in the expanded document view with a language selector, translate button, and copy/download options for the translated text. Useful for cross-border applications (e.g., US user applying for a Spanish visa). Supports bidirectional translation (foreign language to English AND English to foreign language).
...
- **HIPAA Compliance:** Comprehensive security measures including detailed Terms of Service and Privacy Policy, mandatory acceptance during registration, security headers (HSTS, CSP), PHI cache control, rate limiting, audit logging (no IP/PII in logs), session auto-timeout, strong password requirements, database encryption, input validation, path traversal protection, and secure file handling.
